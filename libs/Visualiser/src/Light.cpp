...
Light::Light(LightType _light_type, glm::vec4 _rgba_colour, GLfloat _ambient_intensity, GLfloat _diffuse_intensity)
  : Type(_light_type), Colour(_rgba_colour), AmbientIntensity(_ambient_intensity), DiffuseIntensity(_diffuse_intensity), ShadowMap()
{
...
{
  ProjectionMatrix = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, 1.0f, 20.0f);
  ShadowMap.Init(2048, 2048);
}

glm::mat4 DirectionalLight::CalculateLightTransform() const
{
  return ProjectionMatrix * glm::lookAt(-10.0f * Direction, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
}
...
template<class t_derived_class>
glm::mat4 PointLightBase<t_derived_class>::CalculateLightTransform() const
{
